...
**Settings ▸ Audio Device (ASIO)…** — opens `mabr.ui.AudioSettingsDialog`, edited over `mabr.AudioSettings`: **Testing (loopback, no hardware)** (checked by default — runs the whole engine with no audio device; unchecking it enables the rest of the dialog), the ASIO **Device**, the **Player/Recorder** `[signal timing]` channel mapping, and the **Microphone** input channel used by calibration only. **Commit** applies the settings without closing the dialog — the main window updates behind it, which matters most for **Stimulation only (no recording / no loop-back)**, where committing greys the whole Acquisition panel out and retitles it — and greys itself out again once there is nothing pending; **Cancel** closes, discarding whatever was not committed (and reads **Close** when there is nothing left to discard). Toggling Testing or changing the device rebuilds the worker. Locked for the duration of a schedule.
