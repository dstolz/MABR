...
% The panels are named by the parameters that identify the conditions rather
% than by the raw stimulus ID (see mabr.ui.LivePlot) -- this bank varies Level
% and nothing else, so each panel should carry the level of a block the run
% actually produced, and there should be no panel no block accounts for.
panelTitles = arrayfun(@(a) string(a.Title.String),lp.axMean);
lvls = arrayfun(@(b) double(b.Stim.Meta.Level),ctrl.Session.Blocks(n0+1:end));
for L = unique(lvls(:)')
    assert(any(contains(panelTitles,sprintf('%g dB',L))), ...
        'no live panel is the %g dB condition the run presented (%s)', ...
        L,strjoin(panelTitles,', '));
end
assert(numel(panelTitles) == numel(unique(lvls)), ...
    'the live view showed a panel no block accounts for (%s)', ...
    strjoin(panelTitles,', '));
