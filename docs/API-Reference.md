...
| `paramTable([idx])` | The informative parameters of a set of entries as a table — `Names`, `Values` `[nStim x nP]` row-aligned with `idx`, `Varying` (across *that* subset), `Units` (`kHz`/`dB` for the two the toolbox fixes by name, `''` otherwise), `IDs`. Numeric scalars only: it is a table of dimensions to sort, group, and shade by. Pass a **run's** stimulus indices — what [`LivePlot`](../+mabr/+ui/LivePlot.m) is handed to lay a Frequency × Level run out by parameter |
...
| [`mabr.ui.ProgState`](../+mabr/+ui/ProgState.m) | Enumeration: `Idle`, `PrepBlock`, `Acquire`, `BlockComplete`, `AdvanceBlock`, `SchedComplete`, `Error`. Statics `appearance(state)` (lamp colour + label, shared by every window that shows one) and `isTerminal(state)`. |
...
| [`mabr.ui.ProgressMonitor`](../+mabr/+ui/ProgressMonitor.m) | Schedule progress at a glance: a session/run pair of bars, one bar per stimulus or per parameter value, or a frequency × level heat map with counts. Runs no timer of its own — it rides the controller's live tick and repaints only when the tallies move. |
...
**`LivePlot`** — `LivePlot(parent)` (omit `parent` for its own figure), `update(sweeps,tvec,R,target,bad,info)`, `reset()`, `setFilterText(txt)`. `sweeps` is `[pre post]` as one contiguous segment and `tvec` its time base in seconds; `info.StimIndex`/`.Stimuli`/`.Labels` say which stimulus evoked each sweep (omit it for a single pooled mean), and `info.Params` — a [`StimulusSet.paramTable`](../+mabr/+stim/StimulusSet.m) row-aligned with `.Stimuli` — lets the view label, order, group, and colour those conditions by the parameters that actually vary. Display settings are the public properties `Layout` (`'overlay'`/`'separate'`/`'grid'`/`'stacked'`), `GroupBy` (`''` auto / `'none'` / a parameter name), `TimeBase` (ms, default `[-2 10]`), `AmpMode` (`'each'`/`'common'`/`'manual'`), `ManualLimit` (volts), `ErrorBand` (`'none'`/`'std'`/`'sem'`/`'ci'`) and `ConfidenceLevel` (default 0.95) — the control strip writes the first five and the right-click menu the last two. See [Viewing Data](Viewing-Data.md#liveplot).

**`ProgressMonitor`** — `ProgressMonitor(parent)` (omit `parent` for its own `uifigure`), `listenTo(controller)`, `attach(schedule,stimuli)`, `refresh(force)`, `reset()`, `headerText()`, `isvalidView()`. Display settings are the public properties `View` (`'simple'`/`'bars'`/`'heatmap'`), `GroupBy` (`'Stimulus'` or any parameter name), `HeatX`/`HeatY`, `Labels` (`'counts'`/`'percent'`/`'none'`), `AlwaysOnTop`, and `MinInterval` (the repaint rate limit, default 0.2 s) — the window's control strip writes exactly these. Read-only `Counts`/`Targets` are the last tally, per stimulus, and the drawn objects (`TrackPatch`, `FillPatch`, `ValueText`, `HeatImage`, `HeatText`) are public to read. The denominator is every presentation `Schedule.Runs` currently holds, so artifact make-up enlarges it; the numerator is `Schedule.RunCounts` plus, mid-run, that run's sweeps paired against `Schedule.runSequence`. See [Viewing Data](Viewing-Data.md#progressmonitor).
