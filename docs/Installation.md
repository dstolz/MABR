...
**The logger.** MABR's second submodule, [granary](https://github.com/dstolz/granary) at `external/granary`, is where every message MABR prints and every line of the daily log comes from. The same `git submodule update --init` fetches it. Unlike stimgen it is not optional: without it MABR launches with a warning, prints to the command window, and writes **no log file** — which is the one thing you want when something goes wrong, so fetch it.
...
- `.error_logs/` — one text log per day, written by granary. It holds **more** than the command window shows: the console is gated by `GVerbosity` and the file by `GLogVerbosity`, which defaults to logging everything, so turning the console quiet does not throw away the detail that explains a failure. `mabr.log.logFile` names the current file; `granary.setLogDir` moves the folder if the toolbox sits on a read-only or synced share.
...
`MABR.m` calls [`mabr.log.configure`](../+mabr/+log/configure.m) right after `genpath`, before anything logs, which is what points granary at `<root>/.error_logs` and the `MABR` preference group. Every process that logs calls it — the acquisition and compute workers included, since granary's settings are per-MATLAB-session and a worker is a separate process.
