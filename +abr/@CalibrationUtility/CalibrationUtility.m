    ...
    methods (Static)
        ...
        function timer_Runtime(T,event,app)
            try
                ...
                BH = double(app.Runtime.BufferIndex(2));
                ...
                switch app.Runtime.BackgroundState
                    case abr.stateAcq.COMPLETED
                        app.Calibration.ADC.Data = app.Runtime.mapSignalBuffer.Data(1:BH);
                        shadowSamples = round(0.1*app.Calibration.ADC.SampleRate);
                        app.Calibration.ADC.SweepOnsets = app.Runtime.find_timing_onsets(1,BH,shadowSamples);
                        stop(T);
                        return

                    case abr.stateAcq.ACQUIRE
                        data = app.Runtime.mapSignalBuffer.Data(LB:BH);
                        app.Calibration.ADC = app.Calibration.ADC.appendData(data);

                        % find stimulus onsets in timing signal
                        shadowSamples = round(0.1*app.Calibration.ADC.SampleRate);
                        idx = app.Runtime.find_timing_onsets(LB,BH,shadowSamples);
