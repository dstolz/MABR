...
- `mabr.ui.App` — the programmatic uifigure view (replaces `abr.ControlPanel`); `createComponents` builds five titled panels (Session / Stimulus / Presentation / Acquisition / Run) from one `panelGrid` helper and a shared `LabelWidth`, so the fields align across panel borders; panels carry explicit pixel heights because `'fit'` does not see through a `uipanel` to its nested grid. Owns the presentation settings (bank, per-stimulus repetitions, strategy, ISI); `buildSchedule` is the single place they become a `Schedule`, shared by the plan-summary preview and `onStart` so they cannot drift. It opens **both viewers at launch** and lays them out beside itself; a `uitoolbar` with lettered glyphs raises them (**L** live plot, **T** trace organizer) and opens the wiki (**?**). Because the organizer is always open and keeps itself current through `BlockReady`, **T** is a pure raise — it backfills from the `Session` only when building the organizer from scratch, so it cannot discard a loaded `.torg` view.
